...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-2bfd287a";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-2bfd287a">'+'<span class="tile-face-2bfd287a tile-front-2bfd287a"></span>'+'<span class="tile-face-2bfd287a tile-back-2bfd287a"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2bfd287a")){return;}
var back=tile.querySelector(".tile-back-2bfd287a");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-2bfd287a");if(isMine){tile.classList.add("boom-2bfd287a");}
if(dim){tile.classList.add("dimmed-2bfd287a");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-2bfd287a")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-2bfd287a");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2bfd287a")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-2bfd287a");void panelEl.offsetWidth;panelEl.classList.add("jolt-2bfd287a");}
endRound();}
